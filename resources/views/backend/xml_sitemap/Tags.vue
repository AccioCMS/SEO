<template>
    <div>
        <form class="form-horizontal form-label-left" id="store">
            ...
            <div class="postTypeContainer" v-for="postType in postTypesList" v-if="postType.hasTags">
                ...
                <div class="form-group">
                    <label class="control-label col-md-2 col-sm-2 col-xs-12">Exclude tags of {{ postType.name }}: </label>
                    <div class="col-md-8 col-sm-8 col-xs-12">
                        <multiselect
                                v-model="tagsIgnoredInSitemap[postType.slug]"
                                :options="tagsOptions"
                                :multiple="true"
...
<script>
    export default{
        ...
        methods:{
            ...
            searchTags(query, postTypeSlug, postTypeID){
                this.postOptions = [];
                if(!this.isLoading[postTypeSlug] && query.length > 0){
                    this.isLoading[postTypeSlug] = true;
                    this.$http.get(this.basePath+'/'+this.$route.params.adminPrefix+'/'+this.$route.params.lang+'/json/tags/'+postTypeID+'/search/'+query)
                        .then((resp) => {
                            this.tagsOptions = resp.body.list;
